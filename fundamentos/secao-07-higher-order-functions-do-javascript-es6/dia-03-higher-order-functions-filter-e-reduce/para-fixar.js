...
const sumEvenNumbre = numbers.filter((number) => number % 2 === 0)
.reduce((acc, curr) => acc + curr);

console.log(sumEvenNumbre);

const sumEvenNumberOneFunction = numbers.reduce((acc, curr) => {
    if (curr % 2 === 0) { return acc + curr };
    return acc;
});
console.log(sumEvenNumberOneFunction);
...
console.log(estudantes.map((student) => ({
  name: student.nome,
  materia: student.materias.reduce((acc, subject) => (acc.nota > subject.nota ? acc : subject))
})));